...
$ExitCode       = 0       # Standard exitcode 0 (erfolg); Bei fehler wird es 1
$FailedServices = @()     # Leere Liste für fehlgeschlagene Dienste 
$RestartedCount = 0       # neu gestartet
...
$Summary        = ("{0} Dienste geprueft, {1} neu gestartet, {2} fehlgeschlagen, Exit={3}" -f `
                   $TotalChecked, $RestartedCount, $FailedServices.Count, $ExitCode)

$max = 200    # Aufgabe 2.2 : max. 200 Zeichen
if ($Summary.Length -gt $max) { $Summary = $Summary.Substring(0,$max) }
# die ersten 200 Zeichen nehmen
