...
$RegPath = 'HKLM:\SOFTWARE\CYBERDYNE\Exceptions\General\StoppedServices'
$DynamicExcluded = @()
if (Test-Path $RegPath) {
    $props = (Get-ItemProperty -Path $RegPath).PSObject.Properties | Where-Object { $_.Name -notmatch '^PS' }
    foreach ($p in $props) {
        if ($p.Value) { $DynamicExcluded += [string]$p.Value } else { $DynamicExcluded += [string]$p.Name }
    }
}
...
foreach ($s in $services) {
    ...
    if ($StaticExcluded -contains $svc.Name -or $StaticExcluded -contains $svc.DisplayName) {
        Write-Host ("[SKIP] Dienst '{0}' wurde mittels statischer Ausnahme ausgeschlossen." -f $svc.Name)
        $ExcludedStaticTotal++
        continue
    }

    if ($DynamicExcluded -contains $svc.Name -or $DynamicExcluded -contains $svc.DisplayName) {
        Write-Host ("[SKIP] Dienst '{0}' wurde mittels dynamischer Ausnahme ausgeschlossen." -f $svc.Name)
        $ExcludedDynamicTotal++
        continue
    }

    if ($svc.Status -eq 'Running') {
        Write-Host ("[OK] Dienst '{0}' laeuft bereits." -f $svc.Name)
        continue
    }
